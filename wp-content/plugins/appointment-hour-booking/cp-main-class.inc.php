...
class CP_AppBookingPlugin extends CP_APPBOOK_BaseClass {
    ...
    protected $table_items = "cpappbk_forms";
            ...
            {    
                ...
                switch ($fields[$j]) {
                    ...
                    case 'data':
                        ...
                        break;    
                    case 'paid':
                        echo ($selection[$i][3]['paid']?__('Yes','cpappb'):'&nbsp;');
                        break;
                    default:
    ...
    function insert_public_item() {
        ...
        if (CP_APPBOOK_DEFER_SCRIPTS_LOADING)
        {
            ...
            '{"pub":true,"identifier":"'.('_'.$this->print_counter).'","messages": {
            	                	"required": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_is_required', CP_APPBOOK_DEFAULT_vs_text_is_required),'cpappb')).'",
                                    "maxapp": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_maxapp', CP_APPBOOK_DEFAULT_vs_text_maxapp),'cpappb')).'",
                                    "language": "'.str_replace(array('"'),array('\\"'),$calendar_language).'",
                                    "date_format": "'.str_replace(array('"'),array('\\"'),$this->get_option('date_format', '')).'",
            	                	"email": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_is_email', CP_APPBOOK_DEFAULT_vs_text_is_email),'cpappb')).'",
            	                	"datemmddyyyy": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_datemmddyyyy', CP_APPBOOK_DEFAULT_vs_text_datemmddyyyy),'cpappb')).'",
            	                	"dateddmmyyyy": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_dateddmmyyyy', CP_APPBOOK_DEFAULT_vs_text_dateddmmyyyy),'cpappb')).'",
            	                	"number": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_number', CP_APPBOOK_DEFAULT_vs_text_number),'cpappb')).'",
            	                	"digits": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_digits', CP_APPBOOK_DEFAULT_vs_text_digits),'cpappb')).'",
            	                	"max": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_max', CP_APPBOOK_DEFAULT_vs_text_max),'cpappb')).'",
            	                	"min": "'.str_replace(array('"'),array('\\"'),__($this->get_option('vs_text_min', CP_APPBOOK_DEFAULT_vs_text_min),'cpappb')).'",
        ...
        <script type="text/javascript">
         var cp_hourbk_cancel_label = '<?php echo str_replace("'", "\'", __("cancel",'cpappb')); ?>';
         var cp_hourbk_cost_label = '<?php echo str_replace("'", "\'", __("Cost",'cpappb')); ?>';
    ...
    function insert_button() {
            ...
            var cpappbk_counter = 0;
          	function loadWindow(){
          		...
          		$(' <div title="Appointment Hour Booking"><div style="padding:20px;">'+
          		   'Select Calendar:<br /><select id="cpappbk_calendar_sel'+cpappbk_counter+'" name="cpappbk_calendar_sel'+cpappbk_counter+'"><?php echo $options; ?></select>'+
          		   '</div></div>'
          		  ).dialog({
                      ...
                      buttons: [
                          {text: "Insert", click: function() {
          						if(send_to_editor){
                                    ...
                                    send_to_editor('[<?php echo $this->shorttag; ?> id="'+id+'"]');
          						}
          						$(this).dialog("close");
          				}}
                      ]
        ...
        print '<a href="javascript:cpappbk_appointments_fpanel.open()" title="'.__('Insert Appointment Hour Booking','cpappb').'"><img hspace="5" src="'.plugins_url('/images/cp_form.gif', __FILE__).'" alt="'.__('Insert  Appointment Hour Booking','cpappb').'" /></a>';
   ...
   function data_management_loaded() {
    ...
    function data_management() {
        ...
        if(!empty($_POST['cp_app_action']))
        {
            $formid = $_POST['formid'];
            $field = $_POST['formfield'];
            
            $myrows = $wpdb->get_results( $wpdb->prepare("SELECT posted_data FROM ".$wpdb->prefix.$this->table_messages." where formid=%d", $formid) );
            $tmp2 = array();
            for ($i=0; $i < count($myrows); $i++)
            {
                ...
                for($k=0; $k<count($data["apps"]); $k++) 
                    if ( (!isset($data["apps"][$k]["cancelled"]) || $data["apps"][$k]["cancelled"] == '') && 
                        ( !isset($data["apps"][$k]["field"]) || @$data["apps"][$k]["field"] == $field ) 
                        )
                    {
                        $slot = $data["apps"][$k]["slot"];                              
                        $tmp2[] = array("d"=>$data["apps"][$k]["date"] ,"h1"=>intval(substr($slot,0,2)),"m1"=>intval(substr($slot,3,2)),"h2"=>intval(substr($slot,6,2)),"m2"=>intval(substr($slot,9,2)));
                    }
            }
        ...
        if( $redirect )
        {        
            header("Location: ".$this->get_option('fp_return_page', CP_APPBOOK_DEFAULT_fp_return_page));
            exit();
        }    
    }

    
    function extract_appointments($form,$data,$sequence)
    {
        $apps = array();
    ...
    function get_appointments_text($apps)
    {
        $text = '';
        foreach($apps as $app)
            $text .= " - ".$app["date"]." ".$app["slot"]." (".$app["service"].")\n";
        return $text;    
    }
